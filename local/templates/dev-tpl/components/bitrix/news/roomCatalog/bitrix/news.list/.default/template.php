...
<?php if ($page != "/"): ?>
	<div class="rooms-section__title page-header">Номера</div>
<?php endif; ?>
						...
						<? foreach ($gallery as $photo): ?>
							<div class="swiper-slide">
								<div class="img-container hover-slider__img">
									...
									?>
									<img src="<?= $img['SRC'] ?>" alt="<?= $arItem['NAME'] ?>" class="">
								</div>
							</div>
						<? endforeach ?>
					</div>
	...
	<?php if ($page != "/"): ?>
		<section class="room-form">
			<div class="room-form__container">
				<div class="room-form__bg-text">
					<p>СКИДКА</p>
				</div>
				<div class="room-form__text">
					<div class="room-form__text-discount">
						скидка 10%
					</div>
					<div class="room-form__descr">
						Оставьте заявку на бронирование<br> прямо сейчас и получите скидку
					</div>
				</div>
				<div class="room-form__right">
					<form action="/local/ajax/index.php" method="POST" class="js-form room-form__form">
						<div class="input-wrapper">
							<p class="error-text">Некорректный ввод</p>
							<input placeholder="Ваше имя" id="name" name="name" type="text"
								class="text-input room-form__input">
						</div>
						<div class="input-wrapper">
							<p class="error-text">Некорректный ввод</p>
						...
						<button class="btn-right-arr btn-right-arr--white room-form__submit">
							<p>Заказать звонок</p>
					...
					<div class="room-form__policy"><span>Нажимая кнопку, вы соглашаетесь </span> <a href="/policy/">на
							обработку
							персональных
							данных</a></div>
				</div>
